...
//  myGLCD.setFont((uint8_t*)ArialNumFontPlus);
  myGLCD.setFont((uint8_t*)arial_bold);
  myGLCD.setColor(100, 255, 100);
  myGLCD.setBackColor(0, 0, 0);
  myGLCD.drawBitmap (0, 0, 64, 64, (unsigned int*)logo);
...
void printText() {
  sprintf(timeBuf, "%s", "Text goes here");
...
void incomingMessage(const MyMessage &message)
{
  ...
  Serial.println(message.data);
    ...
    LastMessage=lstate;
    gw.saveState(EPROM_LIGHT_STATE, LastMessage);
  }
}
